    ...
    <div className={`bg-white text-gray-900 ${padding}`} style={{ fontFamily }}>
      ...
      {experience.length > 0 && (
        <Wrap {...aProps(0.2)} className={sectionSpacing}>
          ...
          <TimelineItems items={experience} color={color} animated={animated} type="experience" />
        </Wrap>
      )}
      {education.length > 0 && (
        <Wrap {...aProps(0.3)} className={sectionSpacing}>
          <SectionTitle icon={GraduationCap} title="Education" color={color} />
          <TimelineItems items={education} color={color} animated={animated} type="education" />
        </Wrap>
      )}
            ...
            <div className="space-y-1.5">
              {languages.map((l: any) => (
                <div key={l.id} className="flex justify-between text-[10px]">
                  <span>{l.name}</span><span className="text-gray-400">{l.proficiency}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="flex-1 bg-white p-7 text-gray-900">
        ...
        {experience.length > 0 && (
          <Wrap {...aProps(0.2)} className="mb-6">
            <SectionTitle icon={Briefcase} title="Experience" color={goldAccent} />
            <TimelineItems items={experience} color={goldAccent} animated={animated} type="experience" />
          </Wrap>
        )}
        {education.length > 0 && (
          <Wrap {...aProps(0.3)} className="mb-6">
            <SectionTitle icon={GraduationCap} title="Education" color={goldAccent} />
            <TimelineItems items={education} color={goldAccent} animated={animated} type="education" />
          </Wrap>
        )}
...
const CreativeSplit = ({ data, color, fontFamily, animated, showPhoto, photoRadius }: any) => {
  ...
  return (
    <div className="flex min-h-[800px]" style={{ fontFamily }}>
      ...
      {/* Clean white side */}
      <div className="flex-1 bg-white p-7 text-gray-900">
        {p.summary && <Wrap {...aProps(0.1)} className="mb-6"><p className="text-xs leading-relaxed text-gray-600">{p.summary}</p></Wrap>}
        {experience.length > 0 && (
          <Wrap {...aProps(0.2)} className="mb-6">
            <SectionTitle icon={Briefcase} title="Experience" color={color} />
            <TimelineItems items={experience} color={color} animated={animated} type="experience" />
    ...
    <div className="bg-white text-gray-900 p-10" style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif" }}>
      <Wrap {...aProps(0)} className="mb-10">
        <div className="flex items-end gap-5">
          {showPhoto && (
            <div className="w-16 h-16 overflow-hidden shrink-0" style={{ borderRadius: photoRadius }}>
              <img src={p.profileImage} alt={p.fullName} className="w-full h-full object-cover" />
            </div>
          )}
          <div>
          ...
          {experience.map((exp: any) => (
            <div key={exp.id} className="mb-4 pb-4 border-b border-gray-50">
              <div className="flex justify-between items-baseline">
                ...
                <span className="text-[10px] text-gray-400">{formatDate(exp.startDate)} — {exp.current ? 'Present' : formatDate(exp.endDate)}</span>
              </div>
        ...
        <Wrap {...aProps(0.3)} className="mb-8">
          ...
          {education.map((edu: any) => (
            <div key={edu.id} className="mb-3">
              ...
              <p className="text-[11px] text-gray-400">{edu.institution} · {formatDate(edu.startDate)} — {edu.current ? 'Present' : formatDate(edu.endDate)}</p>
            </div>
          ))}
        </Wrap>
      )}
      {skills.length > 0 && (
        <Wrap {...aProps(0.4)} className="mb-8">
          <h3 className="text-[10px] uppercase tracking-[0.2em] text-gray-300 mb-3 font-medium">Skills</h3>
          <div className="flex flex-wrap gap-1.5">
            {skills.map((s: any) => (<span key={s.id} className="text-[10px] px-2 py-0.5 border border-gray-200 rounded text-gray-600">{s.name}</span>))}
          </div>
        </Wrap>
      )}
      {languages.length > 0 && (
        <Wrap {...aProps(0.5)} className="mb-8">
          <h3 className="text-[10px] uppercase tracking-[0.2em] text-gray-300 mb-3 font-medium">Languages</h3>
          <div className="flex flex-wrap gap-3 text-[11px] text-gray-600">
            {languages.map((l: any) => (<span key={l.id}>{l.name} ({l.proficiency})</span>))}
          </div>
        </Wrap>
      )}
      <HobbiesSection hobbies={hobbies} color={color} animated={animated} />
    ...
    <div className="bg-[#0d1117] text-[#c9d1d9] p-8" style={{ fontFamily: "'Segoe UI', sans-serif" }}>
      <Wrap {...aProps(0)} className="mb-6 pb-4 border-b border-[#21262d]">
        <div className="flex items-center gap-4">
          {showPhoto && (
            <div className="w-16 h-16 overflow-hidden shrink-0" style={{ borderRadius: photoRadius, border: '2px solid #30363d' }}>
              <img src={p.profileImage} alt={p.fullName} className="w-full h-full object-cover" />
            </div>
          )}
          <div>
      ...
      {languages.length > 0 && (
        <Wrap {...aProps(0.4)} className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {languages.map((l: any) => (<span key={l.id} className="text-[10px] px-2 py-0.5 rounded border border-[#30363d] text-[#c9d1d9]">{l.name} · {l.proficiency}</span>))}
          </div>
        </Wrap>
      )}
  ...
  return (
    <div className="bg-white text-gray-900 p-8" style={{ fontFamily }}>
      ...
      <Wrap {...aProps(0)} className="text-center mb-8 pb-6 border-b-2" style={{ borderColor: color }}>
        {showPhoto && (
          <div className="w-24 h-24 mx-auto mb-3 overflow-hidden" style={{ borderRadius: photoRadius, boxShadow: `0 0 0 4px ${color}` }}>
            <img src={p.profileImage} alt={p.fullName} className="w-full h-full object-cover" />
          </div>
        )}
  ...
  return (
    <div className="bg-white text-gray-900" style={{ fontFamily }}>
      ...
      <Wrap {...aProps(0)} className="p-8 pb-6 mb-6" style={{ backgroundColor: color }}>
        <div className="flex items-center gap-5">
          {showPhoto && (
            <div className="w-20 h-20 overflow-hidden shrink-0 bg-white/20" style={{ borderRadius: photoRadius }}>
              <img src={p.profileImage} alt={p.fullName} className="w-full h-full object-cover" />
            </div>
          )}
          <div>
            ...
            <div className="flex flex-wrap gap-3 mt-2 text-[10px] text-white/70">
              {p.email && <span>{p.email}</span>}
              {p.phone && <span>{p.phone}</span>}
              {p.location && <span>{p.location}</span>}
            </div>
      ...
      <div className="px-8 pb-8">
        {p.summary && <Wrap {...aProps(0.1)} className="mb-6"><p className="text-xs leading-relaxed text-gray-600 text-lg font-light italic border-l-4 pl-4" style={{ borderColor: color }}>"{p.summary}"</p></Wrap>}
        {experience.length > 0 && (
          <Wrap {...aProps(0.2)} className="mb-6">
        ...
        {education.length > 0 && (
          <Wrap {...aProps(0.3)} className="mb-6">
            ...
            {education.map((edu: any) => (
              <div key={edu.id} className="mb-3">
                <h4 className="font-bold text-sm">{edu.degree}{edu.field ? `, ${edu.field}` : ''}</h4>
        ...
        <div className="grid grid-cols-2 gap-6">
            ...
            <Wrap {...aProps(0.35)}>
              ...
              {skills.map((s: any) => (
                <div key={s.id} className="mb-2">
                  ...
                  <div className="h-1.5 rounded-full bg-gray-100"><div className="h-full rounded-full" style={{ width: `${s.level}%`, backgroundColor: color }} /></div>
                </div>
              ))}
            </Wrap>
          )}
          {languages.length > 0 && (
            <Wrap {...aProps(0.4)}>
              ...
              {languages.map((l: any) => (
                <div key={l.id} className="text-[11px] mb-1"><span className="font-medium">{l.name}</span> — <span className="text-gray-400">{l.proficiency}</span></div>
              ))}
            </Wrap>
          )}
        </div>
...
const ClassicCorporate = ({ data, color, fontFamily, animated, showPhoto, photoRadius }: any) => {
  ...
  return (
    <div className="bg-white text-gray-900 p-8" style={{ fontFamily: serifFont }}>
      <Wrap {...aProps(0)} className="text-center mb-6 pb-4 border-b-2 border-gray-900">
        ...
        <div className="flex justify-center flex-wrap gap-x-4 mt-2 text-[11px] text-gray-600">
          {p.email && <span>{p.email}</span>}
          {p.phone && <span>{p.phone}</span>}
          {p.location && <span>{p.location}</span>}
        ...
        <Wrap {...aProps(0.2)} className="mb-5">
          ...
          {experience.map((exp: any) => (
            <div key={exp.id} className="mb-3">
              <div className="flex justify-between"><h4 className="font-bold text-sm">{exp.position}</h4><span className="text-[10px] text-gray-500">{formatDate(exp.startDate)} — {exp.current ? 'Present' : formatDate(exp.endDate)}</span></div>
              <p className="text-xs italic text-gray-600">{exp.company}</p>
              {exp.description && <p className="text-[11px] text-gray-700 mt-1 leading-relaxed">{exp.description}</p>}
            </div>
          ))}
        </Wrap>
      )}
        ...
        <Wrap {...aProps(0.3)} className="mb-5">
          ...
          {education.map((edu: any) => (
            <div key={edu.id} className="mb-2">
              <div className="flex justify-between"><h4 className="font-bold text-sm">{edu.degree}{edu.field ? `, ${edu.field}` : ''}</h4><span className="text-[10px] text-gray-500">{formatDate(edu.startDate)} — {edu.current ? 'Present' : formatDate(edu.endDate)}</span></div>
      ...
      {skills.length > 0 && (
        <Wrap {...aProps(0.4)} className="mb-5">
          <h3 className="text-sm font-bold uppercase tracking-wide border-b border-gray-300 pb-1 mb-3">Skills</h3>
          <p className="text-[11px] text-gray-700">{skills.map((s: any) => s.name).join(' · ')}</p>
        </Wrap>
      )}
      {languages.length > 0 && (
        <Wrap {...aProps(0.45)} className="mb-5">
          <h3 className="text-sm font-bold uppercase tracking-wide border-b border-gray-300 pb-1 mb-3">Languages</h3>
          <p className="text-[11px] text-gray-700">{languages.map((l: any) => `${l.name} (${l.proficiency})`).join(' · ')}</p>
        </Wrap>
      )}
...
const CVPreview = () => {
  ...
  return (
    <div id="cv-output" className="max-w-[210mm] mx-auto shadow-xl rounded-lg overflow-hidden">
      <TemplateComponent
        data={data}
        color={color}
